...
The same policy now has deterministic application-format diagnostics for the qualified PMS→PBX subset. These validate only source-backed syntax boundaries: CHK extension/name shape, LMT amount shape, DND status, GRP field width/content, lowercase two-letter `LNG` language coding, exact `MW` status spacing, `RSTn`, `NAM1`-`NAM4` index/name placement, and argument-free `AREYUTHERE`/`GRS`/`END`. Format findings do not assert that a syntactically valid extension exists in a property's inventory and do not promote an ambiguous/reverse-direction command. This expands technician diagnostics without changing the six-dimensional row status or evidence class.
