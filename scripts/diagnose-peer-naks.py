...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Correlate inbound protocol NAK controls with the nearest defensible outbound "
            "transaction without inferring transport or changing compatibility claims."
        )
    )
    parser.add_argument("capture", type=Path, help="JSON capture list or object containing a 'captures' list")
    parser.add_argument("--transport", required=True, choices=("tcp", "serial", "unknown"))
    parser.add_argument(
        "--evidence-class",
        required=True,
        choices=(
    ...
    parser.add_argument("--output", type=Path, help="Write deterministic JSON to this file instead of stdout")
    return parser
