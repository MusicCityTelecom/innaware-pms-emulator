...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Correlate source-backed PhoneSuite serial PMS receive timeout/NAK "
            "conditions without inferring checksum cause, serial defaults, retry "
            "policy, or compatibility promotion."
        )
    )
    parser.add_argument(
        "capture",
    ...
    parser.add_argument(
        ...
        help="Must be serial for the registered PhoneSuite PMS-to-PBX row",
    )
    parser.add_argument(
        "--pms-direction",
        required=True,
        choices=("rx", "tx"),
        help="Local capture direction that independently represents PMS-originated traffic",
    )
    parser.add_argument(
        "--evidence-class",
        required=True,
...
def _load_capture(path: Path) -> list[object]:
    ...
    raise ValueError(
        "capture JSON must be a list or an object containing a 'captures' list"
    )
